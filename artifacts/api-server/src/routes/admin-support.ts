...
export function enhanceAdminDashboardSupportLink(req: Request, res: Response, next: NextFunction) {
  ...
  res.send = ((body?: any): Response => {
    ...
    if (typeof body === "string" && body.includes("Support Tickets") && !body.includes("data-support-dashboard-replies")) {
      ...
      const supportScript = `<script>(function(){function humanSize(bytes){if(!bytes&&bytes!==0)return'';if(bytes<1024)return bytes+' B';if(bytes<1024*1024)return Math.round(bytes/1024)+' KB';return(bytes/1024/1024).toFixed(1)+' MB';}function wire(form){if(!form||form.dataset.supportAttachmentWired==='1')return;form.dataset.supportAttachmentWired='1';var input=form.querySelector('.support-attachment-input');var base64=form.querySelector('input[name="attachmentBase64"]');var name=form.querySelector('input[name="attachmentName"]');var mime=form.querySelector('input[name="attachmentMimeType"]');var status=form.querySelector('[data-support-attachment-status]');if(!input||!base64||!name||!mime)return;input.addEventListener('change',function(){var file=input.files&&input.files[0]?input.files[0]:null;if(!file){base64.value='';name.value='';mime.value='';if(status)status.textContent='Selected file will expire after 24 hours.';return;}var reader=new FileReader();reader.onload=function(){base64.value=String(reader.result||'');name.value=file.name||'support-attachment';mime.value=file.type||'application/octet-stream';if(status)status.textContent='Selected: '+(file.name||'attachment')+' ('+humanSize(file.size)+') — expires in 24 hours.';};reader.readAsDataURL(file);});}window.__earnDailyWireSupportAttachmentForm=wire;var forms=[].slice.call(document.querySelectorAll('form[action*="/api/admin/support/"][action$="/close"]'));forms.forEach(function(closeForm){var cell=closeForm.parentElement;if(!cell||cell.querySelector('[data-support-reply-form]'))return;var csrf=closeForm.querySelector('input[name="_csrf"]');var replyForm=document.createElement('form');replyForm.method='post';replyForm.action=closeForm.action.replace(/\/close$/,'/reply');replyForm.className='stack';replyForm.setAttribute('data-support-reply-form','true');if(csrf)replyForm.appendChild(csrf.cloneNode(true));var returnTo=document.createElement('input');returnTo.type='hidden';returnTo.name='returnTo';returnTo.value='/admin/dashboard';replyForm.appendChild(returnTo);if(!closeForm.querySelector('input[name="returnTo"]')){var closeReturn=document.createElement('input');closeReturn.type='hidden';closeReturn.name='returnTo';closeReturn.value='/admin/dashboard';closeForm.appendChild(closeReturn);}replyForm.innerHTML += '<textarea class="input" name="reply" rows="3" required placeholder="Write reply to user" style="min-width:220px;margin-bottom:6px"></textarea>';
        replyForm.innerHTML += '${supportAttachmentFields().replace(/`/g, "\\`")}' ;
        replyForm.innerHTML += '<div class="actions"><button class="btn green" type="submit">Send Reply</button></div>';cell.insertBefore(replyForm,closeForm);closeForm.style.marginTop='8px';wire(replyForm);});document.querySelectorAll('form[action*="/api/admin/support/"][action$="/reply"]').forEach(wire);})();</script>`;
      nextBody = body
